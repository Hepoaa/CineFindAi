...
const HeartIcon: React.FC<{className: string, isFavorite: boolean}> = ({className, isFavorite}) => (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill={isFavorite ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth={1.5} className={className} aria-hidden="true">
  ...
  return (
    ...
    >
      <div className="relative">
        ...
        <button onClick={handleFavoriteClick} aria-label={item.isFavorite ? `Remove ${title} from favorites` : `Add ${title} to favorites`} className="absolute bottom-2 right-2 bg-black/70 backdrop-blur-sm p-2 rounded-full text-brand-accent opacity-0 group-hover:opacity-100 transition-all duration-300 hover:scale-110">
